...
// async function extractGPXInformation(gpxData) {
...
// async function readGpxFile(gpxFilePath){
...
async function readGpxFile(gpxFilePath) {
  try {
    const data = await readFile(gpxFilePath, 'utf8');
    const result = await parseString(data);
    const gpxData = result.gpx; // Assuming the root element is <gpx>
    const processedData = await extractGPXInformation(gpxData);
